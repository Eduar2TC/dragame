...
class _LevelOneState extends State<LevelOne> {
                    ...
                    child: Container(
                        ...
                        child: Padding(
                          padding: EdgeInsets.all(
                              0), //Agrega padding a los elementos de imagen
                          child: Column(
                            //Columna izquierda
                            children: <Widget>[
                              //arreglo de widgets rows de los elementos Imagenes Origen
                              Row(
                                mainAxisAlignment:
                                    MainAxisAlignment.spaceAround,
                                children: retornaListaDeItems(itemsOrigen),
                                /* 
                                 itemsOrigen.map((item){ //item elemento de la lista
                                    return Container(
                                      child: Draggable <ItemModel>(
                                              data: item,
                                            //imagen arrastrando
                                              feedback: Opacity(opacity: 0.5, child: Image(image: AssetImage(item.imagen) )), // drag image con opacidad
                                              //imagen origen cuando se arrastra el elemento imagen
                                              childWhenDragging: Image(image: AssetImage(item.imagen), color: Color.fromARGB(255, 60, 60, 60)),
                                              child: Image( //-->original imagen
  ...
  List<Widget> rowCol() {
    ...
    List<Widget> lista = [];
    return new List<Widget>.generate(list.length, (index) => Text(list[index]));
  }

  List<Widget> retornaListaDeItems(List<dynamic> listaItems) {
    return listaItems.map((item) {
      //item elemento de la list
      return Column(
        children: <Widget>[
          Draggable<ItemModel>(
            data: item,
            ...
            feedback: Opacity(
                opacity: 0.5,
                child: Image(
                    image: AssetImage(item.imagen))), // drag image con opacidad
            //imagen origen cuando se arrastra el elemento imagen
    ...
    /* new Row(children: strings.map((item) => new Text(item)).toList()); */
  }
